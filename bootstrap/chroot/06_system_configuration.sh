...
echo "system    /dev/disk/by-partlabel/cryptsystem  none    discard,no-read-workqueue,no-write-workqueue,x-initrd.attach" >>"/etc/crypttab"
...
sed -i '/^#DBPath/a\DBPath=/usr/lib/pacman/' /etc/pacman.conf
...
# that should result in the system using zram over
...
echo "$SWAP_FILE none swap sw,pri=0 0 0" >> /etc/fstab
...
systemctl enable swap-swapfile.swap
